...
虽然没有直接证据，但 Modder 们普遍认为上述问题正是 MCP 引入两种不同的映射名的理由：
  ...
  - 形如 `func_12345_a`、`field_67890_b` 的 Serage 名，以当年 MCP 的发起人 Serage 命名。没错，他现在是 Mojang 员工，负责 Java 版 Minecraft 的开发。它的特点是“对于某个签名没有变化的方法或字段来说，其 Serage 名会始终保持一致”。
  - 有特定含义，一般情况下都是英文词组的 MCP 名。
  - 虽然不是映射名，但一般都会把某个 Serage 名或 MCP 名对应的混淆名作“Notch 名”，以 Minecraft 的创始人 Notch 命名。

下表给出了基于 `stable_39` 的 Notch-Serage-MCP 名对照表，可以注意到类名没有独立存在的 Serage 名：

|Notch (1.12)|Serage|MCP|
|:------|:------|:------|
